...
export default function RecruiterDashboard() {
  ...
  const [filterOptions, setFilterOptions] = useState<{
    ...
    companies: string[];
    sources: string[];
  }>({ roleCategories: [], companies: [], sources: [] });
  ...
  // Client-side cache timestamps to avoid redundant refetches
  const lastFetch = useRef<Record<string, number>>({});
  const STALE_MS = 30_000; // 30 seconds

  const isStale = (key: string) => {
    const ts = lastFetch.current[key];
  ...
  const [detailApp, setDetailApp] = useState<
    (ExternalApplication & { score?: ApplicationScore | null }) | null
  >(null);
  ...
  const fetchApplications = useCallback(
    async (force = false) => {
      if (!force && !isStale("applications")) return;
      try {
  ...
  const fetchLeaderboard = useCallback(
    async (force = false) => {
      if (!force && !isStale("leaderboard")) return;
      try {
        const params = new URLSearchParams();
        if (roleFilter) params.set("roleCategory", roleFilter);
        if (companyFilter) params.set("companyName", companyFilter);
  ...
  useEffect(() => {
    Promise.all([fetchStats(true), fetchApplications(true)]).finally(() =>
      setLoading(false),
    );
  ...
  useEffect(() => {
    if (tab === "talent-pool") fetchTalentPool();
  }, [tab, fetchTalentPool]);
  ...
  const handleExportSheet = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const fileName =
          fileNameMatch?.[1] || `zenai_top_${hiringCount}_candidates.xlsx`;
        ...
        toast.success(
          `Downloaded top ${hiringCount} candidates as Excel sheet`,
        );
      } else {
        const data = await res.json();
  ...
  const handleTemporaryRescoreAll = async () => {
    ...
    try {
      ...
      if (!processRes.ok) {
        toast.warning(
          "Re-score queued, but immediate score processing trigger failed.",
        );
      }
  ...
  const hiringLeaderboard = useMemo(() => {
    ...
    if (hiringSourceFilter) {
      filtered = filtered.filter(
        (e) => e.sourcePlatform === hiringSourceFilter,
      );
    }
    if (hiringMinScore > 0) {
    ...
    <>
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 space-y-6">
          {/* Header — hidden on tabs that have their own headers */}
          {tab !== "jobs" && tab !== "schedule" && (
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
              <div>
                ...
                <p className="text-muted-foreground mt-2 text-lg">
                  AI-powered interview analysis & candidate scoring workspace.
                </p>
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  >
                    <div
                      className={`w-10 h-10 rounded-xl bg-white/[0.04] flex items-center justify-center ${color}`}
                    >
              ...
              {overviewTopCandidates.length > 0 && (
                <div className="rounded-xl border border-white/10 bg-card/60 backdrop-blur-sm p-5">
                  ...
                  <div className="space-y-2">
                    {overviewTopCandidates.map(
                      (c: LeaderboardEntry, i: number) => (
                        <div
                          key={c.applicationId}
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground">
                              {c.roleTitle} · {c.companyName}
                            </p>
                          </div>
                          <span
                            className={`text-lg font-bold ${scoreColor(c.overallScore)}`}
                          >
                            {c.overallScore}
                          </span>
                          <span
                            className={`text-[9px] font-bold px-2 py-1 rounded-full border uppercase ${recBadgeColor(c.recommendation)}`}
                          >
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3 items-center">
                ...
                <select
                  ...
                  className="bg-white/[0.04] border border-white/10 rounded-lg px-3 py-2 text-xs text-foreground appearance-none"
                >
                  <option value="">All Roles</option>
                  {filterOptions.roleCategories.map((r) => (
              ...
              <div className="space-y-3 mt-6">
                ...
                {enrichedApps.length === 0 && (
                  <div className="text-center py-16 text-[#888] bg-white/[0.01] rounded-3xl border border-white/5">
                    ...
                    <p className="text-base">
                      No applications found. Import data to populate workspace.
                    </p>
                  </div>
                )}
                ...
                {enrichedApps.map((app) => (
                  <div
                    key={app.id}
                    className="group relative flex flex-col md:flex-row md:items-center gap-4 bg-[#0A0A0A]/40 hover:bg-[#FAFAFA]/[0.03] border border-white/[0.03] hover:border-white/10 p-4 md:p-5 rounded-2xl transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl"
                  >
                    ...
                    <div className="md:w-48 flex flex-col">
                      ...
                      <p className="text-xs text-[#888]">
                        {app.companyName} •{" "}
                        <span className="text-[#A3E635] capitalize">
                          {app.roleCategory || app.sourcePlatform}
                        </span>
                      </p>
                    </div>
                    ...
                    <div className="md:w-24 flex justify-center">
                      {app.score ? (
                        <div
                          ...
                          title={`Score: ${app.score.overallScore}/100`}
                        >
                          <svg className="absolute w-12 h-12 transform -rotate-90">
                            ...
                            <circle
                              ...
                              strokeWidth="2.5"
                              fill="transparent"
                              className={`${scoreColor(app.score.overallScore).replace("text-", "text-")} drop-shadow-[0_0_8px_rgba(163,230,53,0.4)]`}
                              strokeDasharray="125"
                              strokeDashoffset={
                                125 -
                                (125 * Math.min(app.score.overallScore, 100)) /
                                  100
                              }
                              strokeLinecap="round"
                            />
                          ...
                          <span
                            className={`relative z-10 text-xs font-bold ${scoreColor(app.score.overallScore)}`}
                          >
                    ...
                    <div className="md:w-28 flex items-center justify-end gap-1.5">
                      <button
                        ...
                        className="p-2 rounded-full cursor-pointer bg-white/[0.03] hover:bg-white/10 text-[#888] hover:text-white transition-colors"
                      >
                      ...
                      {app.status !== "shortlisted" && (
                        <button
                          ...
                          className="p-2 rounded-full bg-emerald-500/5 hover:bg-emerald-500/20 text-emerald-500 transition-colors"
                        >
                      ...
                      {app.status !== "rejected" && (
                        <button
                          ...
                          className="p-2 rounded-full bg-red-500/5 hover:bg-red-500/20 text-red-500 transition-colors"
                        >
                      ...
                      {app.inviteLink && (
                        <a
                          ...
                          className="p-2 rounded-full bg-blue-500/5 hover:bg-blue-500/20 text-blue-400 transition-colors"
                        >
              ...
              <div className="space-y-3 mt-6">
                ...
                {effectiveLeaderboard.length === 0 && (
                  <div className="text-center py-16 text-[#888] bg-white/[0.01] rounded-3xl border border-white/5">
                    ...
                    <p className="text-base">
                      No scored candidates yet. Interviews are analyzed
                ...
                {effectiveLeaderboard.map((entry) => (
                  <div
                    ...
                    className="group flex flex-col md:flex-row md:items-center gap-4 bg-[#0A0A0A]/40 hover:bg-[#FAFAFA]/[0.03] border border-white/[0.03] hover:border-[#A3E635]/30 p-4 md:p-5 rounded-2xl transition-all duration-300 hover:shadow-[0_0_20px_rgba(163,230,53,0.1)] hover:-translate-y-1"
                  >
                    <div className="md:w-16 flex justify-center md:justify-start pl-2">
                      <span
                        className={`text-3xl font-black ${entry.rank <= 3 ? "text-[#A3E635] drop-shadow-[0_0_12px_rgba(163,230,53,0.4)]" : "text-white/20"}`}
                      >
                        #{entry.rank}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-4 flex-1">
                      <div className="w-12 h-12 rounded-full bg-gradient-to-br from-white/10 to-transparent border border-white/10 flex items-center justify-center overflow-hidden shrink-0 shadow-inner">
                        <span
                          className={`text-lg font-bold capitalize ${entry.rank <= 3 ? "text-white" : "text-white/50"}`}
                        >
                      ...
                      <div className="relative w-12 h-12 flex items-center justify-center flex-shrink-0">
                        <svg className="absolute w-12 h-12 transform -rotate-90">
                          ...
                          <circle
                            ...
                            strokeWidth="2.5"
                            fill="transparent"
                            className={`${scoreColor(entry.overallScore).replace("text-", "text-")} drop-shadow-[0_0_8px_rgba(163,230,53,0.5)]`}
                            strokeDasharray="125"
                            strokeDashoffset={
                              125 -
                              (125 * Math.min(entry.overallScore, 100)) / 100
                            }
                            strokeLinecap="round"
                          />
                        </svg>
                        <span
                          className={`relative z-10 text-xs font-bold ${scoreColor(entry.overallScore)}`}
                        >
                    ...
                    <div className="md:w-36 flex justify-end">
                      <span
                        className={`text-[10px] font-bold px-4 py-1.5 rounded-full border uppercase tracking-wider ${recBadgeColor(entry.recommendation)}`}
                      >
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-[#0A0A0A]/60 to-[#1a1a2e]/40 backdrop-blur-sm p-8">
                ...
                <p className="text-muted-foreground text-sm mb-8">
                  Configure your hiring criteria and download a shortlist of top
                  candidates with their AI-analyzed scores.
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-[#888] uppercase tracking-wider flex items-center gap-1.5">
                      <SlidersHorizontal className="w-3.5 h-3.5" /> Minimum
                      Score
                    </label>
                    <input
                      type="number"
                ...
                <div className="flex items-center gap-4">
                  <button
                    ...
                    disabled={
                      exportingSheet || hiringTopCandidates.length === 0
                    }
                  ...
                  >
                    ...
                    Download Excel Sheet ({hiringTopCandidates.length}{" "}
                    Candidates)
                  </button>
                  <p className="text-xs text-[#888]">
                ...
                <div className="rounded-2xl border border-white/10 bg-[#0A0A0A]/40 backdrop-blur-sm overflow-hidden">
                  <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between">
                    ...
                    <span className="text-xs text-[#888] bg-white/5 px-3 py-1 rounded-full">
                      Top {hiringTopCandidates.length} of{" "}
                      {hiringLeaderboard.length}
                    </span>
                  </div>
                  <div className="divide-y divide-white/[0.03]">
                    {hiringTopCandidates.map((entry, idx) => (
                      ...
                      >
                        <span
                          className={`text-xl font-black w-8 text-center ${idx < 3 ? "text-[#A3E635]" : "text-white/20"}`}
                        >
                          #{idx + 1}
                        </span>
                        ...
                        <div className="flex items-center gap-3">
                          <div className="text-right">
                            <p
                              className={`text-lg font-bold ${scoreColor(entry.overallScore)}`}
                            >
                              {Math.round(entry.overallScore)}
                            </p>
                            <p className="text-[9px] text-[#888] uppercase">
                              Score
                            </p>
                          </div>
                          <span
                            className={`text-[9px] font-bold px-3 py-1 rounded-full border uppercase tracking-wider ${recBadgeColor(entry.recommendation)}`}
                          >
                            {entry.recommendation.replace("_", " ")}
                          </span>
                        </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground text-xs">
                      Candidates actively looking for opportunities
                    </p>
                  </div>
                </div>
              ...
              <div className="space-y-3 mt-6">
                ...
                {!talentLoading && talentPool.length === 0 && (
                  <div className="text-center py-16 text-[#888] bg-white/[0.01] rounded-3xl border border-white/5">
                    ...
                    <p className="text-xs text-[#666]">
                      Candidates who opt-in for visibility will appear here.
                    </p>
                  </div>
                )}

                {!talentLoading &&
                  talentPool.map((entry, idx) => (
                    <div
                      key={entry.id}
                      className="group flex flex-col md:flex-row md:items-center gap-4 bg-[#0A0A0A]/40 hover:bg-[#FAFAFA]/[0.03] border border-white/[0.03] hover:border-purple-500/30 p-4 md:p-5 rounded-2xl transition-all duration-300 hover:shadow-[0_0_20px_rgba(168,85,247,0.1)] hover:-translate-y-1"
                    >
                      <div className="md:w-16 flex justify-center md:justify-start pl-2">
                        <span
                          className={`text-3xl font-black ${idx <= 2 ? "text-purple-400 drop-shadow-[0_0_12px_rgba(168,85,247,0.4)]" : "text-white/20"}`}
                        >
                          #{idx + 1}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-4 flex-1">
                        <div className="w-12 h-12 rounded-full bg-gradient-to-br from-white/10 to-transparent border border-white/10 flex items-center justify-center overflow-hidden shrink-0 shadow-inner">
                          <span
                            className={`text-lg font-bold capitalize ${idx <= 2 ? "text-white" : "text-white/50"}`}
                          >
      ...
      {detailApp && (
        ...
        >
          <div
            className="bg-[#1a1a1f] border border-white/10 rounded-2xl w-full max-w-lg mx-4 p-6 max-h-[85vh] overflow-y-auto"
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="p-3 rounded-lg bg-white/[0.03]">
                  ...
                  <p className="text-primary capitalize">
                    {detailApp.roleCategory}
                  </p>
                </div>
                <div className="p-3 rounded-lg bg-white/[0.03]">
                  <p className="text-muted-foreground">Company</p>
                  <p className="text-foreground font-medium">
                    {detailApp.companyName}
                  </p>
                </div>
                <div className="p-3 rounded-lg bg-white/[0.03]">
                  <p className="text-muted-foreground">Source</p>
                  <p className="text-foreground font-medium capitalize">
                    {detailApp.sourcePlatform}
                  </p>
                </div>
                <div className="p-3 rounded-lg bg-white/[0.03]">
                  <p className="text-muted-foreground">Status</p>
                  <span
                    className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${statusBadge(detailApp.status)}`}
                  >
                    {detailApp.status}
                  </span>
                </div>
              ...
              {detailApp.score && (
                <div className="space-y-3">
                  ...
                  {[
                    ...
                    {
                      label: "Technical",
                      score: detailApp.score.technicalScore,
                    },
                    {
                      label: "Communication",
                      score: detailApp.score.communicationScore,
                    },
                    {
                      label: "Problem Solving",
                      score: detailApp.score.problemSolvingScore,
                    },
                  ].map(({ label, score }) => (
                    <div key={label}>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span
                      className={`text-xs font-bold px-3 py-1 rounded-full border uppercase ${recBadgeColor(detailApp.score.recommendation)}`}
                    >
                  ...
                  {/* Strengths */}
                  {detailApp.score.strengths &&
                    detailApp.score.strengths.length > 0 && (
                      <div className="space-y-1">
                        ...
                        {detailApp.score.strengths.map((s, i) => (
                          <p
                            key={i}
                            className="text-xs text-foreground/70 pl-3 border-l-2 border-emerald-500/30"
                          >
                            {s}
                          </p>
                        ))}
                  ...
                  {/* Weaknesses */}
                  {detailApp.score.weaknesses &&
                    detailApp.score.weaknesses.length > 0 && (
                      <div className="space-y-1">
                        ...
                        {detailApp.score.weaknesses.map((w, i) => (
                          <p
                            key={i}
                            className="text-xs text-foreground/70 pl-3 border-l-2 border-red-500/30"
                          >
                            {w}
                          </p>
                        ))}
              ...
              {!detailApp.score && (
                <div className="p-4 rounded-lg bg-white/[0.02] text-center">
                  <p className="text-xs text-[#888]">
                    No AI analysis available yet. Use &quot;Temp: Re-score
                    All&quot; to analyze this interview.
                  </p>
                </div>
              )}
              ...
              {detailApp.externalJobUrl && (
                <a
                  href={detailApp.externalJobUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 text-xs text-primary hover:underline"
                >
                  <ExternalLink className="w-3.5 h-3.5" /> View original job
                  posting
                </a>
              )}
              ...
              <div className="flex gap-2 pt-2">
                <button
                  onClick={() => {
                    handleStatusChange(detailApp.id, "shortlisted");
                    setDetailApp(null);
                  }}
                  className="flex-1 bg-emerald-500/15 text-emerald-400 text-sm font-medium py-2 rounded-lg hover:bg-emerald-500/25 transition-colors"
                >
                  Shortlist
                </button>
                <button
                  onClick={() => {
                    handleStatusChange(detailApp.id, "rejected");
                    setDetailApp(null);
                  }}
                  className="flex-1 bg-red-500/15 text-red-400 text-sm font-medium py-2 rounded-lg hover:bg-red-500/25 transition-colors"
                >
                  Reject
                </button>
              </div>
